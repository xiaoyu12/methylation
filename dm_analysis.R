...
promoters.dmc.count = DM_count_by_region(promoters.gr)
...
dmc.regions.rat["genes", ]$CpG <- slot(moabs.dmc.genes, "annotation")[1] / gene_len_perc
dmc.regions.rat["exons", ]$CpG<- slot(moabs.dmc.exons, "annotation")[1] / exons_len_perc
dmc.regions.rat["introns", ]$CpG<- slot(moabs.dmc.introns, "annotation")[1] / introns_len_perc
dmc.regions.rat["promoters", ]$CpG<- slot(moabs.dmc.promoters, "annotation")[1] / promoters_len_perc
dmc.regions.rat["up2000", ]$CpG<- slot(moabs.dmc.up2000, "annotation")[1] / up2000_len_perc
dmc.regions.rat["down2000", ]$CpG <- slot(moabs.dmc.down2000, "annotation")[1] / down2000_len_perc
...
# CHG context
moabs.HG.genes <- annotateWithFeature(moabs.HG.DMC, genes$Gene, feature.name = "gene regions")

moabs.HG.exons <- annotateWithFeature(moabs.HG.DMC, gene.parts$exons, feature.name = "exons")

moabs.HG.introns <- annotateWithFeature(moabs.HG.DMC, gene.parts$introns, feature.name = "introns")

moabs.HG.up2000 <- annotateWithFeature(moabs.HG.DMC, up2000, feature.name = "up2000")
moabs.HG.down2000 <- annotateWithFeature(moabs.HG.DMC, down2000, feature.name = "down2000")
moabs.HG.promoters <- annotateWithFeature(moabs.HG.DMC, gene.parts$promoters, 
                                           feature.name = "promoters")

#moabs.HG.Ann <- annotateWithGeneParts(moabs.HG.DMC, gene.parts)

dmc.regions.rat["genes", ]$CHG <- slot(moabs.HG.genes, "annotation")[1] / gene_len_perc
dmc.regions.rat["exons", ]$CHG<- slot(moabs.HG.exons, "annotation")[1] / exons_len_perc
dmc.regions.rat["introns", ]$CHG<- slot(moabs.HG.introns, "annotation")[1] / introns_len_perc
dmc.regions.rat["promoters", ]$CHG<- slot(moabs.HG.promoters, "annotation")[1] / promoters_len_perc
dmc.regions.rat["up2000", ]$CHG<- slot(moabs.HG.up2000, "annotation")[1] / up2000_len_perc
dmc.regions.rat["down2000", ]$CHG <- slot(moabs.HG.down2000, "annotation")[1] / down2000_len_perc
...
ggplot(dmc.regions.rat) + geom_point(aes(x = factor(region, level = region_order), y=CpG, colour="CpG")) +
  ...
  theme_light() +
  scale_y_continuous(limits=c(0, 1.3), n.breaks = 7) +
  labs(x = "Feature Region", y = "Relative DMC Rate") +
