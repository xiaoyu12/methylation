...
DMC_count_in_region <- function(DMCs, region.gr, min_len=1) {
  ...
  dmc.count$density = dmc.count$dmcs * 1000/ dmc.count$len
...
DEG_nonDEG_test <- function(dmc.count) {
  ...
  dmc.count.DEG = dmc.count[dmc.count$ID %in% rownames(annot.DE), ]
  #dmc.count.nonDEG = dmc.count %>% filter (!(ID %in% rownames(annot.DE)))
  dmc.count.nonDEG = dmc.count[!(dmc.count$ID %in% rownames(annot.DE)), ]
...
DEG_up_down_test <- function(dmc.count) {
  ...
  dmc.count.DEG.up = dmc.count[dmc.count$ID %in% rownames(annot.DE.up), ]
  dmc.count.DEG.down = dmc.count[dmc.count$ID %in% rownames(annot.DE.down), ]
...
weighted_mean_std <- function(dmc.count) {
  weighted_stats <- dmc.count %>%
    mutate(density = 1000 *dmcs / len) %>%
    summarize(
      # Weighted mean: sum(dmcs) / sum(len)
      avg_w = weighted.mean(density, len),
      
      # Weighted SD: sqrt( sum(w * (x - mu)^2) / sum(w) )
      sd_w = sqrt(sum(len * (density - avg_w)^2) / sum(len))
    )
  return (c(weighted_stats$avg_w, weighted_stats$sd_w))
} 

# Test for promoter regions
promoters.dmc.count = DMC_count_in_region(moabs.DMC, promoters.gr)
r = DEG_nonDEG_test(promoters.dmc.count)
r$results
r = DEG_up_down_test(promoters.dmc.count)
r$results

#exons.dmc.count = DMC_count_in_region(moabs.DMC, gene.parts$exons)
...
# Test for exon regions
exons <- as.data.frame(gene.parts$exons)
exons.dmc.count <- DMC_count_in_region(moabs.DMC, as(exons, "GRanges"))
exons.dmc.count <- exons.dmc.count %>%    # combining multiple exons of the same gene
  group_by(name) %>%
  mutate(
    start = min(start),
    end = max(end),
...
exons.dmc.count$density = exons.dmc.count$dmcs * 1000/ exons.dmc.count$len

# Test for intron regions
introns <- as.data.frame(gene.parts$introns)
introns.dmc.count <- DMC_count_in_region(moabs.DMC, as(introns, "GRanges"))
introns.dmc.count <- introns.dmc.count %>%   # Combine multiple introns of the same gene
  group_by(name) %>%
  mutate(
    start = min(start),
    end = max(end),
...
introns.dmc.count$density = introns.dmc.count$dmcs * 1000/ introns.dmc.count$len

# Test for up2000 regions
up2000.dmc.count = DMC_count_in_region(moabs.DMC, up2000, min_len = 500)

# Test for down2000 regions
down2000.dmc.count = DMC_count_in_region(moabs.DMC, down2000, min_len = 500)
...
moabs.dmc.genes <- annotateWithFeature(moabs.DMC, genes$Gene, feature.name = "gene regions")
moabs.dmc.exons <- annotateWithFeature(moabs.DMC, gene.parts$exons, feature.name = "exons")
moabs.dmc.introns <- annotateWithFeature(moabs.DMC, gene.parts$introns, feature.name = "introns")
moabs.dmc.up2000 <- annotateWithFeature(moabs.DMC, up2000, feature.name = "up2000")
moabs.dmc.down2000 <- annotateWithFeature(moabs.DMC, down2000, feature.name = "down2000")
moabs.dmc.promoters <- annotateWithFeature(moabs.DMC, gene.parts$promoters, feature.name = "promoters")
...
dmc.regions.rat <- data.frame(region=c("up2000", "promoters", "genes", "exons", "introns",  "down2000"), 
                              CpG=0, CpG_sd=0, CHG=0, CHG_sd=0)
rownames(dmc.regions.rat) <- dmc.regions.rat$region
dmc.regions.rat["genes", c("CpG", "CpG_sd")] <- weighted_mean_std(genes.dmc.count)
dmc.regions.rat["exons", c("CpG", "CpG_sd")] <- weighted_mean_std(exons.dmc.count) 
dmc.regions.rat["introns", c("CpG", "CpG_sd")] <- weighted_mean_std(introns.dmc.count)
dmc.regions.rat["promoters", c("CpG", "CpG_sd")] <- weighted_mean_std(promoters.dmc.count)
dmc.regions.rat["up2000", c("CpG", "CpG_sd")] <- weighted_mean_std(up2000.dmc.count)
dmc.regions.rat["down2000", c("CpG", "CpG_sd")] <- weighted_mean_std(down2000.dmc.count)
...
# CHG context
genes.CHG.count <- DMC_count_in_region(moabs.HG.DMC, genes$Gene)
promoters.CHG.count <- DMC_count_in_region(moabs.HG.DMC, gene.parts$promoters)
up2000.CHG.count <- DMC_count_in_region(moabs.HG.DMC, up2000)
down2000.CHG.count <- DMC_count_in_region(moabs.HG.DMC, down2000)
exons.CHG.count <- DMC_count_in_region(moabs.HG.DMC, gene.parts$exons)
exons.CHG.count <- exons.CHG.count %>%    # combining multiple exons of the same gene
  group_by(name) %>%
  mutate(
    start = min(start),
    end = max(end),
...
introns.CHG.count <- DMC_count_in_region(moabs.HG.DMC, gene.parts$introns)
introns.CHG.count <- introns.CHG.count %>%    # combining multiple exons of the same gene
  group_by(name) %>%
  mutate(
    start = min(start),
    end = max(end),
...
dmc.regions.rat["genes", c("CHG", "CHG_sd")] <- weighted_mean_std(genes.CHG.count)
dmc.regions.rat["exons", c("CHG", "CHG_sd")]<- weighted_mean_std(exons.CHG.count)
dmc.regions.rat["introns", c("CHG", "CHG_sd")]<- weighted_mean_std(introns.CHG.count)
dmc.regions.rat["promoters", c("CHG", "CHG_sd")]<- weighted_mean_std(promoters.CHG.count)
dmc.regions.rat["up2000", c("CHG", "CHG_sd")]<- weighted_mean_std(up2000.CHG.count)
dmc.regions.rat["down2000", c("CHG", "CHG_sd")] <- weighted_mean_std(down2000.CHG.count)
rownames(dmc.regions.rat) <- seq(1,6)
region_order <- dmc.regions.rat$region
# Reshape data for grouped bar chart
plot_data <- data.frame(
  region = rep(dmc.regions.rat$region, 2),
  context = factor(rep(c("CpG", "CHG"), each = nrow(dmc.regions.rat)), levels = c("CpG", "CHG")),
  mean = c(dmc.regions.rat$CpG, dmc.regions.rat$CHG),
  sd = c(dmc.regions.rat$CpG_sd, dmc.regions.rat$CHG_sd)
)

png(filename = "moabs_graphs/DMC_region_rate.png", width=6600, 
    height=3000, res=600)
ggplot(plot_data, aes(x = factor(region, level = region_order), y = mean, fill = context)) + 
  geom_col(position = position_dodge(width = 0.7), width = 0.6) +
  geom_errorbar(aes(ymin = pmax(0, mean - sd), ymax = mean + sd), 
                position = position_dodge(width = 0.7), width = 0.2) +
  scale_fill_manual("", breaks = c("CpG", "CHG"), values = c("CpG"= "red", "CHG" = "blue")) +
